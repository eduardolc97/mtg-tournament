...
import {
  aggregatePauperLeague,
  formatPauperLeagueRankingMessage,
} from '../utils/pauperLeague';
...
import PauperLeaguePresentationDialog from './PauperLeaguePresentationDialog';
...
export default function PauperLeaguePage() {
  ...
  const rows = useMemo(
    () => aggregatePauperLeague(tournaments, year, month),
  ...
  const handleCopyRanking = async () => {
    const message = formatPauperLeagueRankingMessage(rows, year, month);
    try {
    ...
    <div className="min-h-screen min-h-[100dvh] bg-gradient-to-br from-slate-950 via-purple-950 to-slate-900">
      <div className="container mx-auto px-3 py-6 max-w-4xl sm:px-4 sm:py-8">
        ...
        <p className="text-slate-400 text-sm mb-6 max-w-xl">
          Soma de pontos dos campeonatos na modalidade{' '}
          <span className="text-slate-300">Liga Pauper semanal</span>. Vitória = 3
          pts, empate = 1 pt, derrota = 0. Desempate (pontos empatados): média de
          aproveitamento no mês.
        </p>

        <Card className="bg-slate-900/50 border-purple-900/50 backdrop-blur mb-6">
          <CardHeader>
            <CardTitle className="text-white text-base">Período da liga</CardTitle>
          </CardHeader>
        ...
        {loading ? (
              ...
              {excludedFromLeagueCount > 0 ? (
                <>
                  <p>
                    ...
                    {monthLabel(year, month)}, mas nenhum na modalidade Liga Pauper
                    semanal.
                  </p>
                </>
              ) : (
                <p>
                  Nenhum campeonato Pauper em {monthLabel(year, month)}. Crie um
                  torneio escolhendo essa modalidade e o mês/ano.
                </p>
              )}
            </CardContent>
          </Card>
          ...
          <>
            <PauperLeaguePresentationDialog
                ...
                <div className="flex flex-wrap items-start gap-2">
                  <CardTitle className="text-white flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between flex-1 min-w-0">
                    ...
                    <span className="text-sm font-normal text-slate-400 text-right">
                      {eventCount} campeonato{eventCount !== 1 ? 's' : ''} (Liga
                      Pauper)
                    </span>
                  </CardTitle>
                  {rows.length > 0 && (
